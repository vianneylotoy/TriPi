...
class Execution:
	...
	#Sort according to pi the largest in descending order
	def sort_pi(self):
		self.LT_sorted = sorted(self.LT, key=lambda varphi: varphi[0], reverse = True)
	...
	#Display of the list of tasks sorted according to pi the largest
	def display_sortedTask(self):
		self.doc.write("___________________________________________________ Task sorting by pi ______________________________________________________"+'\n')
		for i,ti in enumerate(self.LT_sorted):
			self.tache.pi, self.tache.φi, self.tache.si = ti
			print("Task sorting by pi ->: T[%r] = [%r,%r,%r]" % (i,self.tache.pi, self.tache.φi, self.tache.si))	
			self.doc.write("Task sorting by pi->: T["+str(i)+"] = p"+str(i)+"-> "+str(self.tache.pi)+", φ"+str(i)+"-> "+str(self.tache.φi)+", s"+str(i)+"-> "+str(self.tache.si)+ '\n')
